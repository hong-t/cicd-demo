...
pipeline {
  ...
  environment {
    ...
    successwxmsg = sh(returnStdout: true, script: "filecontent=`cat $JENKINS_HOME/wechat-templates/success_wechat_tmp.md`;content=`echo $content | sed '\'s/JOB_NAME/\"$JOB_NAME\"/\''`")
  }
  tools {
    go 'go'
  }
